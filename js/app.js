...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p>Rating: ${product.rating.rate}stars(rated by ${product.rating.count})</p>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button onclick="loadProductsDetails(${product.id})" id="details-btn" class="btn btn-danger">Details</button></div>
...
const displayProductDetails = product => {
   ...
   const div = document.createElement('div');
   div.innerHTML = `
   <div class="card single-product" style="width: 18rem;">
   <img src="${product.image}" height="150px" width="150px" class="card-img-top" alt="${product.title}">
   <div class="card-body">
